...
function M.sha1_binary(msg)
   return hex_to_binary(M.sha1(msg))
end
...
function M.hmac_sha1(key, text)
   ...
   return M.sha1(key_xord_with_0x5c .. M.sha1_binary(key_xord_with_0x36 .. text))
end

function M.hmac_sha1_binary(key, text)
   return hex_to_binary(M.hmac_sha1(key, text))
end
